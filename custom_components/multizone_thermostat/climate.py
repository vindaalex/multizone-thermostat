        ...
        for hvac_mode in [HVAC_MODE_HEAT, HVAC_MODE_COOL]:
            if hvac_mode in obj:
                ...
                if CONF_PROPORTIONAL_MODE in obj[hvac_mode]:
                    ...
                    if CONF_MASTER_MODE in obj[hvac_mode][CONF_PROPORTIONAL_MODE]:
                        if (
                            CONF_SATELITES
                            not in obj[hvac_mode][CONF_PROPORTIONAL_MODE][
                                CONF_MASTER_MODE
...
class MultiZoneThermostat(ClimateEntity, RestoreEntity):
    ...
    async def async_added_to_hass(self):
        ...
        if self._sensor_out_entity_id:
            self.async_on_remove(
                async_track_state_change_event(
                    ...
                    self._async_track_outdoor_temp,
                )
            )
        ...
        if (
            ...
            self.async_on_remove(
                async_track_time_interval(
                    self.hass,
                    self._async_track_stale_sensor,
        ...
        if self._passive_switch:
            ...
            async_track_utc_time_change(
                self.hass,
                self._async_track_stuck_switch,
                hour=_dt.hour,
                minute=_dt.minute,
                second=_dt.second,
            )
    ...
    async def async_set_hvac_mode(self, hvac_mode):
        ...
        if self._hvac_on.get_pwm_mode:
            await self._async_routine_pwm(self._hvac_on.get_pwm_mode)
    ...
    async def _async_track_indoor_temp(self, event):
    ...
    async def _async_track_outdoor_temp(self, event):
    ...
    async def _async_track_stale_sensor(self, now=None):
    ...
    @callback
    async def _async_track_stuck_switch(self, now):
    ...
    @callback
    async def _async_track_switches(self, event):
